...
## File formats to be converted to WebP, see formats readable by Pillow in: https://pillow.readthedocs.io/en/stable/handbook/image-file-formats.html
INPUT_FORMAT = (".jpg", ".jpeg", ".png", ".bmp", ".dib", ".tif", ".tiff") 
ZIP_TRESHOLD = 1    # Set the minimum amount of INPUT_FORMAT files inside the Zip to mark it for conversion
WEBP_LOSSLESS = False   # True or False for lossless encoding, setting True will disable continuous fallback conversion
WEBP_QUALITY = 85   # 1-100 Encoding quality, higher produces larger size but closer to source quality
WEBP_METHOD = 6     # 1-6 Encoding method, higher takes more time but gives smaller size
...
FALLBACK_RATIO = 0.7    # Maximum filesize ratio of Converted to Original files that will cause the converter...
FALLBACK_INCREMENT = 5  # ...to convert again at decreasing increments of each FALLBACK_INCREMENT for even smaller filesize


def thread_job(Jobs_list, Output_path, WEBP_current_quality):
    '''
    ...
    Takes: - Jobs_list, iterable whose elements are full paths to image files.
           ...
           - WEBP_current_quality, integer of the resulting WEBP saving quality
    NO returns
    '''
    for Job in Jobs_list:
        Converted_filename = os.path.splitext(os.path.basename(Job))[0] + ".webp"
        Converted_path = os.path.abspath(Output_path + os.sep + Converted_filename)
        image_object = Image.open(Job).convert("RGB")
        image_object.save(Converted_path, format="webp", lossless=WEBP_LOSSLESS, quality=WEBP_current_quality, method=WEBP_METHOD)


def thread_manager(Jobs_list_for_T1, Jobs_list_for_T2, Jobs_list_for_T3, Jobs_list_for_T4, Output_path, WEBP_current_quality):
    '''
    ...
    NO returns
    '''
    Thread_obj_1 = threading.Thread(target=thread_job, args=(Jobs_list_for_T1, Output_path, WEBP_current_quality))
    Thread_obj_2 = threading.Thread(target=thread_job, args=(Jobs_list_for_T2, Output_path, WEBP_current_quality))
    Thread_obj_3 = threading.Thread(target=thread_job, args=(Jobs_list_for_T3, Output_path, WEBP_current_quality))
    Thread_obj_4 = threading.Thread(target=thread_job, args=(Jobs_list_for_T4, Output_path, WEBP_current_quality))
    Thread_obj_1.start()
    Thread_obj_2.start()
    Thread_obj_3.start()
    Thread_obj_4.start()
...
def get_WEBP_files_size(Input_path):
    '''
    Get total size of WEBP files within a folder & its subfolder
...
def main():
    print("\n    ===============================================================")
    print("    ||  MULTI-FORMAT .cbz/.zip to WEBP Multi-threaded Converter  ||")
    print("    ===============================================================\n")
    ...
    for Match_path in Match_paths_list:
        ...
        print(f">Converting the images to WEBP with params: lossless={WEBP_LOSSLESS}, quality={WEBP_QUALITY}, method={WEBP_METHOD}")
        ...
        WEBP_current_quality = WEBP_QUALITY
        thread_manager(Jobs_list_for_T1, Jobs_list_for_T2, Jobs_list_for_T3, Jobs_list_for_T4, Converted_path, WEBP_current_quality)
        converted_filesize = get_WEBP_files_size(Converted_path)
        
        ## No need for deletion since Pillow .save will overwrites the file ###################
        if FALLBACK_FLAG == True and WEBP_LOSSLESS != True:
            while converted_filesize > (original_filesize * FALLBACK_RATIO):
                WEBP_current_quality -= FALLBACK_INCREMENT
                if WEBP_current_quality >= 1:
                    print(f"~Re-converting with quality of {WEBP_current_quality} as converted files are larger than the {FALLBACK_RATIO} ratio. ({(converted_filesize/(1024*1024.0)):.2f}MB to {(original_filesize/(1024*1024.0)):.2f}MB)")
